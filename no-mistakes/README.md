...
├── config.local.yaml     NOT tracked: machine/local keys (agent choice, agent paths)
└── README.md             this file

~/.no-mistakes/config.yaml   derived: merge of the two above, regenerated on every sync
```

`~/.no-mistakes/config.yaml` is fully derived (pi model): hand edits die on
the next sync. Which key goes where:

| Key class | Lives in | Examples |
|---|---|---|
| Shared (same on every machine) | `config.yaml` (committed) | timeouts, auto-fix limits, review agents |
| Machine/local (per machine or per day) | `config.local.yaml` (gitignored) | `agent`, `agent_path_override`, `worktree_roots`, `forge_profiles` |
...
```sh
sed -i '' 's/^agent: claude$/agent: opencode/' ~/.claude/no-mistakes/config.local.yaml
uv run --directory ~/.claude/settings-sync sync no-mistakes
...
(commit it like any other skill). From then on:
...
cd ~/.claude && git diff skills/no-mistakes  # usually empty; commit if it changed
```

sync.sh's check-if-missing install probes `command -v no-mistakes`, finds
the updated binary, and skips - no interaction between updating and our sync.
