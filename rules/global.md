...
- **Projects have a `.agents/backlog.md` at the git root** for deferred side topics, maintained via the `backlog` skill.
- **Legacy project documents:** use `.agents/plans/` and `.agents/backlog.md` when present. If only legacy `.claude` documents exist, read them and identify the legacy location. If both exist, reconcile contents before resuming, editing, moving or deleting either; preserve backlog IDs and plan progress.
...
- Never `git commit` or `git push`, even when asked by a skill or subagent, unless the gate below passes.
  ...
  - The gate answers "may I", never "should I": commit/push only when the current task actually calls for it. A set env var makes the action permitted, not wanted - do not commit or push spontaneously just because the gate is open.
