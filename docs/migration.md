...
Native `~/.claude/skills`, `commands`, and `agents` remain real directories. Each shared entry points into the source checkout; local `skills/synced` and `skills/headless-chromium-rootless-libs` stay local. `~/.agents/skills` points to that assembled skills directory.
...
Local verification runs the same script as hosted CI. Implementation branch: `hayden/agent-config-reliability`. Check Git status, remote refs and the branch's workflow runs for current publication and CI status.
